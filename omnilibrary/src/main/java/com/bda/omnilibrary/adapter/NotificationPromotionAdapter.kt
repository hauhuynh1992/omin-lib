...
class NotificationPromotionAdapter(
    val mActivity: Activity,
    private val orders: ArrayList<String>
) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private var currentPositionFocus: Int = 0
    private lateinit var mListener: ItemPromotionListener
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val view =
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_notification_promotions, parent, false)
    ...
    inner class ViewHolder(view: View) :
        ...
        init {
            ...
            view.setOnClickListener {
                mListener.onItemPromotionClicked(absoluteAdapterPosition)
            }
        }
...
//            tv_status.text = mActivity.getString(Functions.getIdOrderStatusData(order.orderStatus))
    ...
    fun setItemListener(listener: ItemPromotionListener) {
        mListener = listener
    }

    interface ItemPromotionListener {
        fun onItemPromotionClicked(position: Int)
    }
}