...
class ThumbsBar @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet?,
    defStyle: Int = 0
) :
    ...
    fun setNumberOfThumbs(numOfThumbs: Int) {
        val newNumOfThumbs = if (numOfThumbs != -1) numOfThumbs else mMeasuredNumOfThumbs
    ...
    protected fun createThumbView(parent: ViewGroup): View {
        ...
        imageView.adjustViewBounds = true // MOD: fit width
        return imageView
    }
    ...
    fun setThumbBitmap(index: Int, bitmap: Bitmap?) {
        ...
        (getChildAt(index) as ImageView).visibility =
    ...
    init {
        ...
        mThumbHeightInPixel =
            (mThumbWidthInPixel / 1.77).toInt() // limit height by 16:9 ratio (vertical videos fix)
        ...
        mHeroThumbHeightInPixel =
            (mHeroThumbWidthInPixel / 1.77).toInt() // limit height by 16:9 ratio (vertical videos fix)
