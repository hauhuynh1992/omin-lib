...
class DotsIndicator(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
    ...
    init {
        ...
        try {
            ...
            this.animatorResId = ta.getResourceId(R.styleable.DotsIndicator_dots_animator,
                R.anim.shophop_scale_with_alpha)
            this.animatorReverseResId =
                ta.getResourceId(R.styleable.DotsIndicator_dots_animator_reverse, 0)
            this.backgroundResId = ta.getResourceId(R.styleable.DotsIndicator_dot_drawable,
                R.drawable.shophop_bg_black_dot)
            this.unselectedBackgroundId =
                ta.getResourceId(R.styleable.DotsIndicator_dot_drawable_unselected,
        ...
        indicatorBackgroundResId =
            if (this.backgroundResId == 0) R.drawable.shophop_bg_black_dot else this.backgroundResId
        ...
        gravity = if (intrinsicGravity >= 0) intrinsicGravity else Gravity.CENTER
    }

    fun setDotDrawable(
        @DrawableRes indicatorRes: Int,
        @DrawableRes indicatorForwardRes: Int,
        ...
        this.backgroundForwardResId = indicatorForwardRes
        indicatorBackgroundResId =
            if (this.backgroundResId == 0) R.drawable.shophop_bg_black_dot else this.backgroundResId
        indicatorUnselectedBackgroundResId =
            if (this.unselectedBackgroundId == 0) this.backgroundResId else this.unselectedBackgroundId
    ...
    fun setDotTintRes(@ColorRes tintRes: Int) = setDotTint(ContextCompat.getColor(context, tintRes))
    ...
    private fun invalidateDots() {
        for (i in 0 until childCount) {
            ...
            if (this.dotTint != 0 && bgDrawable != null) {
                bgDrawable = tint(bgDrawable, this.dotTint)
            }
            indicator.background = bgDrawable
        }
    }

    private fun tint(drawable: Drawable, @ColorInt color: Int): Drawable {
    ...
    private fun internalPageSelected(position: Int) {
        ...
        if (selectedIndicator != null) {
            if (changedByTimer) {
                ...
                selectedIndicator.setBackgroundResource(backgroundForwardResId)
            }
    ...
    private fun addIndicator(orientation: Int, @DrawableRes drawableRes: Int, animator: Animator) {
        ...
        var bgDrawable = ContextCompat.getDrawable(context, drawableRes)
        if (this.dotTint != 0 && bgDrawable != null) {
            bgDrawable = tint(bgDrawable, this.dotTint)
        }
        indicator.background = bgDrawable
    ...
    private fun createAnimatorIn(): Animator {
        ...
        if (this.animatorReverseResId == 0) {
            ...
            animatorIn = AnimatorInflater.loadAnimator(context, this.animatorReverseResId)
        }
        return animatorIn
    }
